...
main_process = None  # Переменная для хранения процесса songs.py

def start_main_script():
    """Запускает songs.py и возвращает объект процесса."""
    global main_process
    if main_process is None or main_process.poll() is not None:
        print("Запуск songs.py...")
        # Updated path to point to the correct location
        main_process = subprocess.Popen(['/home/spedymax/venv/bin/python3', '/home/spedymax/tg-bot/scripts/songs.py'])

def restart_main_script():
    """Перезапускает songs.py."""
    global main_process
    if main_process:
        print("Останавливаю songs.py...")
        main_process.terminate()
        main_process.wait()  # Ожидание завершения
    start_main_script()  # Перезапуск songs.py

@app.route('/update_songs', methods=['POST'])
def update_repo():
    """Обрабатывает вебхук от GitHub для обновления репозитория и перезапуска songs.py."""
    print("Получен POST-запрос на обновление (songs).")
    if request.method == 'POST':
        ...
        try:
            ...
            print("Репозиторий успешно обновлен. Перезапуск songs.py...")
            restart_main_script()  # Перезапускаем songs.py после обновления
            return 'Репозиторий обновлен и songs.py перезапущен.', 200
        except Exception as e:
            print(f"Ошибка обновления репозитория: {e}")
            return f"Ошибка обновления репозитория: {e}", 500

def main_loop():
    """Постоянно проверяет состояние songs.py и перезапускает его при падении."""
    while True:
        try:
...
if __name__ == '__main__':
    ...
    flask_thread = Thread(target=lambda: app.run(host='0.0.0.0', port=5003))
    flask_thread.start()

    # Запускаем проверку на падение songs.py
    main_loop()
