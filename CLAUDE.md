...
## 🗿 Ивент «Месть Пуджинио-Фамозы» + ежедневный данж (2026-09-03)

- **Босс** — `src/services/boss_service.py` (состояние в БД: `boss_events`, `boss_damage_log`) + `src/handlers/boss_handlers.py` (катсцены, закреп с HP, тик 30с, реген 04:00 Kyiv). Ивент «включён», пока есть строка `status='active'`; истекает сам по `ends_at`. Kill switch: `PUDGE_EVENT=false` в `.env`.
- **Урон**: тривия (15), Wordle (20 + 5 за неиспользованную попытку), `/pisunchik` (10), комната данжа (5), мини-пуджик (40). Фазы: ≤66% — Джарвис захвачен на 24ч (инъекция в `_POST_PROMPT` moltbot), ≤33% — ярость (x2). День 7 — Торговец с загадкой (ответ в чат → x2 на 24ч).
- **Админ-команды**: `/pudge_start [hp] [days]` (по умолчанию 2500/14, запускает интро + закреп), `/pudge_stop`, `/pudge_status`, `/pudge_hit <n>`, `/pudge_intro_test` (превью интро в ЛС), `/pudge_merchant_test`.
- **Данж** — `src/services/dungeon_logic.py` (чистая детерминированная логика), `dungeon_service.py` (`dungeon_daily`, `dungeon_runs`), `miniapp/dungeon.html` + эндпоинты `/miniapp/api/dungeon/{state,act}` в `miniapp/app.py`, `/dungeon` и deep-link `?start=dungeon`. Один общий расклад в день на всех; золото данжа тратится только внутри забега. Контент: `assets/data/dungeon_content.json` (лор из чата), интро босса: `plot.json["boss_intro"]`, сцены: `pudge_event.json`.
- Вечерние «правильные ответы» получают блок босса и блок данжа. Плановые посты (Wordle, пророчество, ответы, высер) беззвучные.
- Награды реальные: MVP — уважение Джарвиса неделю (инъекция `respect_persona`) + власть над ботом 2 недели; проигравший — кружок с извинениями Пуджинио на коленях.
