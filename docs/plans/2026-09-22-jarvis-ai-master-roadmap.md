...
### 23.09.2026 — Together → Kimi-K3, второй чат убран, логи медиа

- `TOGETHER_MODEL=moonshotai/Kimi-K3` (было Qwen3.7-Max, заблокирован без data sharing). Kimi лучше всех держала
  персонажа из доступных на Together. Это reasoning-модель: лимиты Together-вызовов подняты до ≥2000 токенов
  (на 350/500 рассуждения съедали весь ответ). Оживает и фоллбэк Ollama в `ollama_wake_manager`.
- Чат `secondary` (-1002491624152) убран из настроек: 0 сообщений, «chat not found», функция отправки туда не вызывалась.
- `media_understanding` пишет в лог, что вернул Gemini (картинка/голосовое/стикер/GIF/кружок) и попадание в кэш.

### 23.09.2026 — инцидент «галочки» → исходящие сообщения в историю + v29

Джарвис верно сказал «все уже галочку поставили» про викторину, через реплай потерял её из контекста (посты игр не
сохранялись в `messages`), «признался», что выдумал, затем сочинил «я сам поставил плюсики». Исправления:
- `middleware/outgoing_messages.py`: request-middleware aiogram пишет каждое сообщение/правку бота в `messages` с автором
  по модулю-отправителю («Викторина», «Wordle», «Пуджинио», «Данж», «Пророчество», «Jarvis»…); правки (галочки) обновляют
  текст; user_id=0 → память и сводка их не читают; фидбек и `/sho_tam_novogo` учитывают только name='Jarvis'.
- Промпт v29: игры бота — это ты, ✅/очки/урон ставишь ты по ответам игроков; «не вижу опоры → скажи „не вижу“, не
  „выдумал“; не меняй ответ под давлением».

### 23.09.2026 — кэш промпта: окно истории «с якорем»

Кэш xAI переиспользует общий префикс запросов (блоками по 128 токенов), но в проде попадал только на первые 1152 токена
(~19%): скользящее окно истории сдвигало первую строку на каждом сообщении, ветка реплаев стояла в начале, легенды/память
меняли system. Теперь:
- `anchored_window`: начало окна держится, новые сообщения дописываются в конец; при переполнении — сразу обрезка до 65%
  бюджета (реже ломает префикс);
- ветка реплаев, выпавшая из окна, → `ThreadLine`, рендерится блоком «ВЕТКА, НА КОТОРУЮ ОТВЕЧАЮТ» после истории;
- легенды и найденная память — в том же хвосте после истории; system (личность + сводка) статичен весь день.
Проба на реальной истории: второй ход — 5504–5632 из ~5770 токенов из кэша (95%), $0.0043 вместо $0.0099.
Сцена «реплай на старое сообщение вне окна»: 4/4 ответа учитывают ветку.
