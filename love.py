...
def get_commands(message):
    ...
    startuem = False
    commands = ""
    if current_date >= datetime.date(2025, 2, 28):
        commands += '/count - узнать сколько дней вы вместе\n'
    if current_date >= datetime.date(2025, 2, 29):
        commands += '/100reasons - узнать 100 причин\n'
    if current_date >= datetime.date(2025, 2, 30):
        commands += '/trivia - викторина\n/answers - ответы на викторину\n'
    if current_date >= datetime.date(2025, 2, 31):
        commands += '/invite - вас пригласили на свидание!\n'
    if current_date == datetime.date(2025, 2, 32):
        startuem = True
