...
class TournamentManager:
    def __init__(self, state_file, bot, chat_id, db_manager, spotify_client):
    ...
    def load_tournament_state(self):
        ...
        try:
            if os.path.exists(self.state_file):
                ...
                logging.info(f"Tournament state loaded: Round {self.current_round_index + 1}, match {self.current_matchup_index + 1}")
                return True
            else:
                logging.info("No tournament state file found")
                return False
        except Exception as e:
            logging.error(f"Error loading tournament state: {str(e)}")
            return False
    ...
    def save_tournament_state(self):
        ...
        try:
            ...
            logging.info("Tournament state saved")
            return True
        except Exception as e:
            logging.error(f"Error saving tournament state: {str(e)}")
            return False

    def initialize_tournament(self, song_pools):
    ...
    def create_pairs(self, songs):
        ...
        try:
            ...
            logging.error(f"Error creating pairs: {str(e)}")
            raise

    def post_daily_matchup(self):
        ...
        try:
            ...
            song1_info = self.spotify_client.get_track_info(matchup[0]["track_uri"])
            song2_info = self.spotify_client.get_track_info(matchup[1]["track_uri"])
            
            # Download and send audio for both songs
            file1 = self.spotify_client.download_song(matchup[0]["track_uri"])
            if not file1:
            ...
            # Send first song
            try:
                self.bot.send_audio(self.chat_id, audio=open(file1, 'rb'))
                self.spotify_client.delete_file(file1)
            except Exception as e:
            ...
            file2 = self.spotify_client.download_song(matchup[1]["track_uri"])
            if not file2:
            ...
            # Send second song
            try:
                self.bot.send_audio(self.chat_id, audio=open(file2, 'rb'))
                self.spotify_client.delete_file(file2)
            except Exception as e:
            ...
            logging.info(f"Matchup posted: {matchup[0]['track_uri']} vs {matchup[1]['track_uri']}")
            
            return True
        except Exception as e:
            logging.error(f"Error posting matchup: {str(e)}")
            return False

    def _check_existing_matchup(self, matchup):
        ...
        try:
            ...
            logging.error(f"Error checking existing matchup: {str(e)}")
            return None

    def handle_vote(self, user_id, vote_value):
        ...
        from config import MIN_VOTES_TO_FINALIZE
    ...
    def handle_admin_vote(self, voter_id, vote_value):
    ...
    def notify_non_voters(self, participants):
    ...
    def finalize_matchup(self):
        ...
        song_info = self.active_matchup.get(
            ...
            self.spotify_client.get_track_info(winner_song["track_uri"])
        )
    ...
    def record_matchup_result(self, matchup, winner, vote1=0, vote2=0):
    ...
    def build_next_round(self):
        ...
        if len(winners) == 1:
            # Get winner song info
            song_info = self.spotify_client.get_track_info(winners[0]['track_uri'])
    ...
    def get_leaderboard(self):
        ...
        for i, song in enumerate(leaderboard, 1):
            song_info = self.spotify_client.get_track_info(song["track_uri"])
            text += (
