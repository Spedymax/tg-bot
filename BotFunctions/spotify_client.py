...
class SpotifyClient:
    def __init__(self, client_id, client_secret, download_dir):
    ...
    def get_tracks_from_playlist(self, playlist_url):
        ...
        try:
            ...
            logging.error(f"Error fetching playlist tracks: {str(e)}")
            raise

    @retry(stop=stop_after_attempt(2), wait=wait_exponential(multiplier=1, min=2, max=5))
    def download_song(self, track_uri):
        ...
        try:
            ...
            logging.info(f"Downloading song {track_uri} to {unique_dir}")
            ...
            if result.returncode != 0:
                logging.error(f"Failed to download {track_uri}: {result.stderr}")
                return None
            
            # Get the downloaded file
            files = os.listdir(unique_dir)
            if not files:
                logging.error(f"No files found after download for {track_uri}")
                return None
    ...
    def delete_file(self, file_path):
        ...
        try:
            if os.path.exists(file_path):
                ...
                logging.warning(f"File {file_path} doesn't exist and can't be deleted.")
        except Exception as e:
            logging.error(f"Failed to delete file {file_path}: {str(e)}")

    def create_playlist_from_tracks(self, track_uris, playlist_name):
