...
def handle_callback(call):
    ...
    for player_id in pisunchik:
        markup.add(types.InlineKeyboardButton(get_player_name(player_id), callback_data=f"selecplayer_{action}_{player_id}"))

    bot.send_message(call.message.chat.id, f"Select a player to {action.replace('_', ' ')}:", reply_markup=markup)


@bot.callback_query_handler(func=lambda call: call.data.startswith("selecplayer"))
