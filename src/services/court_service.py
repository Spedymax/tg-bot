...
Карты должны быть ЗАЦЕПКАМИ из мира этого конкретного подсудимого — детали, аномалии, факты,
которые звучат подозрительно ИЛИ оправдывающе в зависимости от интерпретации.

Правила:
- Специфичны для подсудимого: для ИИ — логи, запросы, версии, токены; для человека — бытовые детали его жизни; для животного — следы, поведение, запахи
- ДВУСМЫСЛЕННЫ: их можно использовать как обвинение И как защита ("запрос в 3:17 ночи" — подозрительно или просто пик нагрузки?)
- ЗАПРЕЩЕНО: категоричные факты, прямые признания вины, однозначные улики — только зацепки
- Прокурорские — слегка тревожные, наводящие на мысль о вине
- Адвокатские и свидетельские — нейтральные или двусмысленно оправдывающие
...
Будь ироничным — это не совсем обычный суд. Если аргументы обеих сторон примерно равны по нелепости — можешь оправдать по нестандартной причине. Наказание должно звучать как настоящий приговор, но может быть неожиданным или комичным. Финальный блок начинается с жирного ВИНОВЕН или НЕ ВИНОВЕН, затем — приговор с характером."""
