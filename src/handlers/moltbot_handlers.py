...
- Верни ТОЛЬКО текст нового summary в формате markdown, без пояснений, без обёртки в ```"""
    ...
    async def _call_together(self, sender_name: str, user_text: str,
        ...
        hard_rules = (
            ...
            "3. ДЛИНА ОТВЕТА: Отвечай КОРОТКО — 1-3 предложения в обычном чате. "
            "Максимум 4-5 если тема реально интересная. "
            "НИКОГДА не пиши стены текста, монологи, эссе. "
            "Ты в чате, а не на сцене. Соизмеряй длину ответа с длиной сообщения.\n"
            "4. НЕ ВЫДУМЫВАЙ события, которых не было. Не придумывай чужие сторис, фото, инстаграмы. "
            "Если не знаешь — скажи что не знаешь, но в своём стиле.\n"
            "5. Приятная в общении. Подкалывай легко и с юмором, НЕ унижай и НЕ читай нотации. "
            "Если дерзят — короткая дерзкая ответочка, а не лекция на 10 абзацев.\n"
            "6. Если для ответа нужна актуальная информация из интернета "
